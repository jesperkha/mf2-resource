 ...
 */
export function parseCST(source: string, onError: (range: CST.Range, msg: string) => void): CST.Resource;

// resource = line *(newline line)
// line = section-head / entry / comment / empty-line
export function parseCST(source_: string, onError_: (range: CST.Range, msg: string) => void): CST.Resource {
 ...
 */
const contentRegExp = /[\t\x20-\x5B\x5D-\x7E\u{A0}-\u{2027}\u{202A}-\u{D7FF}\u{E000}-\u{10FFFF}]/u;
...
const escIdRegExp = /[\x21-\x2F\x3A-\x40\x5B-\x60\x7B-\x7E\xA1-\xBF\xD7\xF7\u2010-\u2027\u2030-\u205E\u2190-\u2BFF]/;
const escValueChars = "{|}";
function parseEscape(context: "id" | "value"): CST.Escape {
    ...
    if (hexLength) {
        ...
        const ok = context === "id" ? escIdRegExp.test(ch) : escValueChars.includes(ch);
...
function parseLineEnd(type: string) {
    ...
    if (ch === "\n") {
        ...
        const msg = type === "empty-line" ? "Content with unexpected indent" : "Unexpected content at line end";
        onError([pos, end], msg);
    }
}
